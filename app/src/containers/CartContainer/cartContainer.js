...
class CartDetail extends React.Component {
  ...
  componentDidMount() {
    ...
    if (
      ...
      getCookie("wishListUniqueId") !== undefined &&
      getCookie("wishListUniqueId") !== null &&
      getCookie("wishListUniqueId") !== ""
    ) {
  ...
  render() {
    ...
    return !!cartData.cartItems && !!cartData.cartItems.length ? (
      <>
        ...
        <section className="cartDetails">
          ...
          <div className="cartItem">
            <div className="cartHeadDetails">
              {!isMobile() ? (
                <h2 className="title">
                  ...
                  <span className="cartCount">
                    {cartData.cartTotalItems} {ITEMS}
                  </span>
                </h2>
              ) : (
                <div className="checkout-top-hedaer">
                  <a href="/" className="mob-header-logo">
                    <MWebLogo width="24" height="24" />
                  </a>
                  <h2 className="title">
                    {YOUR_CART}
                    <span className="cartCount">
              ...
              <Pincode getCartDetails={this.props.getCartDetails} />
            </div>
            <ul className="cartItemList">
              {cartData.cartItems.map((itemData, index) => {
                ...
                return (
                  <li
                    className={`prodList${outOfStock ? " outOfStockList" : ""}`}
                    key={`${index}-pro`}
                  >
                    ...
                    >
                      <figure className="prodImg">
                        <img
                          ...
                          src={
                            itemData.thumbnail
                              ? `${imagePrefix}${itemData.thumbnail}`
                      ...
                      {!itemData.freeGift ? (
                        !isMobile() && (
                          <span className="price">
                            ₹{formatPrice(itemData.offerPrice)}
                          </span>
                        )
                      ) : (
                      ...
                      {!itemData.freeGift
                        ? !isMobile() && (
                            <span className="shipping">
                              {SHIPPING_CHARGES}{" "}
                              {itemData.shippingCharges === 0
                                ? `${FREE}`
                                : itemData.shippingCharges}
                            </span>
                          )
                        : ""}
                    </div>
            ...
            <div className="summary">
              <p className="cartTotal">
                ...
                <span className="val">
                  ...
                  ₹
                  {formatPrice(
                    cartData.orderSummary.totalAmount === ""
                      ? "0"
                      : cartData.orderSummary.totalAmount
                  )}
                </span>
              ...
              {!!cartData.orderSummary.productDiscount && (
                <p className="prodDisc">
                  ...
                  <span className="val">
                    - ₹{formatPrice(cartData.orderSummary.productDiscount)}
                  </span>
                </p>
              )}
              {!!cartData.orderSummary.orderDiscount && (
                <p className="orderDisc">
                  ...
                  <span className="val">
                    - ₹{formatPrice(cartData.orderSummary.orderDiscount)}
                  </span>
                </p>
              )}
              {!!cartData.orderSummary.shippingCharges == "0" ? (
                <p className="shipping">
                  ...
                  <span className="shippingMsg">
                    {cartData.orderSummary.shippingMessage}
                  </span>
                </p>
              ) : (
                <p className="shipping">
                  ...
                  <span className="val">
                    -₹
                    {formatPrice(cartData.orderSummary.shippingCharges)}
                  </span>
                </p>
              )}
              <div className={!isMobile() ? "emiInfo" : "emiInfo mob-emi-info"}>
                <EMIVal
                  getCartDetails={this.props.getCartDetails}
                  price={cartData.orderSummary.netAmount}
                />
              ...
              {!isMobile() ? (
                <p className="totalAmt">
                  ...
                  <span className="val">
                    ₹{formatPrice(cartData.orderSummary.netAmount)}
                  </span>
                  <span className="savingText">
                    {YOU_SAVED}{" "}
                    <span className="savedAmt">
                      ₹{formatPrice(cartData.orderSummary.saving)}
                    </span>
                  </span>
                </p>
              ) : (
                ...
                <div className="checkout-btn-floater">
                  <div className="total-amount">
                    <div className="net-amount-box">
                      ₹{formatPrice(cartData.orderSummary.netAmount)}
                      <span className="total-amount-text">{TOTAL_AMOUNT}</span>
                    </div>
                  </div>
