...
import { Button, Form, FormGroup, Row, Col, Label, Alert } from 'react-bootstrap';
import { registartionAPI, generateOTPAPI, accessToken } from '../../../public/constants/constants';
...
class RegisterWithEmailMobile extends React.Component {
    ...
    handleValidation(obj, errorType) {
        ...
        } else if (!regexEmail.test(obj.userId) && this.props.registrationType !== registerWithMobileNum) {
            this.setState({
                errorMessageUserId: 'Please enter valid Email Id',
            });
            isValidate = false;

        } else if (!regexMobileNo.test(obj.userId) && this.props.registrationType === registerWithMobileNum) {
            this.setState({
