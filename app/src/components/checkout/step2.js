...
export class Step2Component extends React.Component {
  ...
  callPinApi = (val, errorField) => {
    return new Promise((resolve, reject) => {
      ...
      axios
        .get(`${PinToCityAPI}${val ? val : defPin}`, {
          headers: {
            store_id: storeId,
            access_token: token
          }
  ...
  bPincodeFocusOut() {
    if (
      this.state.binputText_pincode !== "" &&
      this.state.binputText_pincode.length === 6
    ) {
      ...
      axios
        ...
        .then(response => {
          if (this.state.same_bill == false && !this.props.isLoggedIn) {
            this.saveBillAdd().then(billAddId => {
              this.setState({
  ...
  saveGST = orderId => {
    return new Promise((resolve, reject) => {
      if (
        this.state.inputText_gst !== "" &&
        this.state.inputText_gst.length == 15
      ) {
        let token = appCookie.get("accessToken");
        var data = {
          order_id: orderId,
  ...
  handleloginContinue = () => {
    ...
    if (this.state.same_bill == false) {
      this.saveBillAdd().then(addressId => {
        var selected_address = this.state.addressList[this.state.selected_add];
        this.setState({
  ...
  async onLoginSave(event) {
    ...
    if (this.state.same_bill == false) {
      ...
      if (!validateState(this.state.binputText_state)) {
        ...
        this.setState({
          berror_state: true,
          berrorMessage_state: !this.state.binputText_state
            ? `${REQUIRED_FIELD}`
            : `${ENTER_VALID_STATE}`
        });
      ...
      if (!validateCityDistrict(this.state.binputText_city)) {
        ...
        this.setState({
          berror_city: true,
          berrorMessage_city: !this.state.binputText_city
            ? `${REQUIRED_FIELD}`
            : `${VALID_CITY_DISTRICT}`
        });
        validateBillingAddress = false;
      ...
      if (!validateAddress(this.state.binputText_address)) {
        ...
        this.setState({
          berror_address: true,
          berrorMessaget_address: !this.state.binputText_address
            ? `${REQUIRED_FIELD}`
            : `${VALID_ADDRESS}`
        });
        validateBillingAddress = false;
      ...
      if (!validateAddress(this.state.binputText_address1)) {
        ...
        this.setState({
          ...
          berrorMessaget_address1: !this.state.binputText_address1
            ? `${REQUIRED_FIELD}`
            : `${VALID_ADDRESS}`
        });
        validateBillingAddress = false;
      ...
      if (!validateEmailId(this.state.binputText_email)) {
        ...
        this.setState({
          berror_email: true,
          berrorMessage_email: !this.state.binputText_email
            ? `${REQUIRED_FIELD}`
            : `${VALID_EMAIL}`
        });
        validateBillingAddress = false;
      ...
      if (!validatePindcode(this.state.binputText_pincode) || !bPincodeValid) {
        ...
        this.setState({
          berror_pincode: true,
          berrorMessage_pincode: !this.state.binputText_pincode
            ? `${REQUIRED_FIELD}`
            : `${VALID_PINCODE}`
        });
        validateBillingAddress = false;
      ...
      if (!validateMobileNo(this.state.binputText_number)) {
        ...
        this.setState({
          berror_number: true,
          berrorMessage_number: !this.state.binputText_number
            ? `${REQUIRED_FIELD}`
      ...
      if (!validateFullName(this.state.binputText_name)) {
        ...
        this.setState({
          berror_name: true,
          berrorMessage_name: !this.state.binputText_name
            ? `${REQUIRED_FIELD}`
  ...
  async onSavebuttonClick(event) {
    ...
    if (!validateAddress(this.state.inputText_address)) {
      ...
      this.setState({
        error_address: true,
        errorMessaget_address: !this.state.inputText_address
          ? `${REQUIRED_FIELD}`
          : `${VALID_ADDRESS}`
      });
      validateBillingAddress = false;
    ...
    if (!validateEmailId(this.state.email)) {
      ...
      this.setState({
        error_email: true,
        errorMessage_email: !this.state.email
          ? `${REQUIRED_FIELD}`
          : `${VALID_EMAIL}`
      });
      validateBillingAddress = false;
    ...
    if (!validateMobileNo(this.state.phone)) {
      ...
      this.setState({
        error_number: true,
        errorMessage_number: !this.state.phone
          ? `${REQUIRED_FIELD}`
          : "Please enter valid mobile number"
      });
      validateBillingAddress = false;
  ...
  renderAddressList = () => {
    if (this.state.addressList && this.state.addressList.length > 0) {
      ...
      return <div className="noAddress">No Saved address</div>;
    }
  };
  ...
  render() {
    return (
      <>
        {isMobile() && <div className="checkout-title">Deliver To</div>}
        <div className="col-md-8 checkout_wrapper">
          {this.state.pinPop ? (
            <PinChangePopup
              cancel={this.cancelPinPop}
              currentPinPro={this.state.currentPin}
              currentAddressPro={
                this.state.addressList !== null
                  ? this.state.addressList[this.state.selected_add]
                  : null
              }
            />
          ) : (
          ...
          <div className="listRow clearfix">
            ...
            {!this.props.isLoggedIn && !isMobile() ? (
              <div className="action-button">
                <button
                  onClick={this.handleChangeMobile}
                  className="btn-block btn-blackbg"
                >
                  Change
                </button>
              </div>
              ...
              <div className="leftBox bgGrey">
                ...
                {this.props.isLoggedIn ? (
                  <div className="verticalTab">
                    <div
                      className={`add_tab ${this.state.saved_add}`}
                      onClick={this.savedAddActive}
                    >
                      <div
                        style={
                    ...
                    <div
                      className={`add_tab ${this.state.new_add}`}
                      onClick={this.newAddActive}
                    >
                      <div>New Address</div>
                    </div>
                  </div>
            ...
            <div className="rightBox">
              ...
              {!this.props.isLoggedIn || this.state.new_add ? (
                <div className={isMobile() && "add-new-address-form"}>
                    ...
                    <div className="col-md-12">
                      <div className="form-div clearfix div-error">
                        ...
                        {this.state.error_address ? (
                          <div className="error-msg">
                            {this.state.errorMessaget_address}
                          </div>
                        ) : null}
                      </div>
                  ...
                  <div className="row">
                    ...
                    {this.props.isLoggedIn && isMobile() ? (
                      <div className="col-md-12">
                        ...
                        <div className="col-xs-12 col-md-12 address-submit">
                          <button
                            className="btn-blackbg btn-block"
                            disabled={this.state.isSaveBtnDisabled}
                            onClick={this.onSavebuttonClick.bind(this)}
                          >
                            {!isMobile() ? "Submit" : "Save Address"}
                          </button>
                        </div>
                      </div>
                    ) : (
                    ...
                    {this.state.new_add_error ? (
                      <div className="col-md-12 error-box">
                        <div className="error-msg">
                          {this.state.new_add_msg}
                        </div>
                      </div>
                    ) : null}
                  </div>
              ...
              {!this.state.new_add || isMobile() ? (
                <div>
                      ...
                      <div className="bill-address customCheckbox clearfix">
                        <div className="input_box">
                          <input
                            ...
                            type="checkbox"
                            name="billing"
                            checked={this.state.same_bill}
                            onChange={this.handleSameBill}
                          />
                  ...
                  {!this.state.same_bill ? (
                    <div className="bill_div">
                      <div className="row">
                        <div className="col-md-12">
                          <p class="label-enter-billing">
                        ...
                        <div className="col-md-6 colpaddingRight">
                          <div className="form-div clearfix div-error">
                            <Input
                              ...
                              handleChange={this.handleInput}
                            />
                            {this.state.berror_name ? (
                              <div className="error-msg">
                                {this.state.berrorMessage_name}
                              </div>
                            ) : null}
                          </div>
                        </div>
                        ...
                        <div className="col-md-6 colpaddingRight">
                          <div className="form-div clearfix div-error">
                            <Input
                              ...
                              id="bpin"
                              name="bpin"
                              value={this.state.binputText_pincode}
                              handleChange={this.handleInput}
                              focusOut={this.bPincodeFocusOut.bind(this)}
                            />
                            {this.state.berror_pincode ? (
                              <div className="error-msg">
                                {this.state.berrorMessage_pincode}
                              </div>
                            ) : null}
                          </div>
                        </div>
                        <div className="col-md-6">
                          <div className="form-div clearfix div-error">
                            <Input
                              ...
                              handleChange={this.handleInput}
                            />
                            {this.state.berror_email ? (
                              <div className="error-msg">
                                {this.state.berrorMessage_email}
                              </div>
                            ) : null}
                          </div>
                        </div>
                        ...
                        <div className="col-md-12">
                          <div className="form-div clearfix div-error">
                            <Input
                              ...
                              maxLength={30}
                            />
                            {this.state.berror_address ? (
                              <div className="error-msg">
                                {this.state.berrorMessaget_address}
                              </div>
                            ) : null}
                          </div>
                        </div>
                        ...
                        <div className="col-md-12">
                          <div className="form-div clearfix div-error">
                            <Input
                              ...
                              maxLength={40}
                            />
                            {this.state.berror_address1 ? (
                              <div className="error-msg">
                                {this.state.berrorMessaget_address1}
                              </div>
                            ) : null}
                          </div>
                        </div>
                        ...
                        <div className="col-md-6 colpaddingRight">
                          <div className="form-div clearfix div-error">
                            <Input
                              ...
                              name="bcity"
                              id="bcity"
                              value={this.state.binputText_city}
                            />
                        ...
                        <div className="col-md-6">
                          <div className="form-div clearfix div-error">
                            <Input
                              ...
                              title="State"
                              name="bstate"
                              value={this.state.binputText_state}
                              id="bstate"
                            />
                    ...
                    <div className="col-md-12">
                      <div className="form-div clearfix div-error">
                        <Input
                          ...
                          value={this.state.inputText_gst}
                          name="gst"
                          handleChange={this.handleInput}
                        />
                        {this.state.error_gst ? (
                          <div className="error-msg">
                            {this.state.errorMessage_gst}
                          </div>
                        ) : null}
                      </div>
                    </div>
                ...
                <div>
                      ...
                      <div className="havePassword customCheckbox clearfix">
                        <div className="input_box">
                          <input
                            ...
                            checked={this.state.defaultAddress}
                          />
                          <label className="lblCheck" htmlFor="checkboxBill" />
                        </div>
                  ...
                  <div className="row">
                    <div
                      className={!isMobile() ? "col-md-6" : "col-xs-6 col-md-6"}
                    >
                      <button
                        className="btn-cancelborder btn-block"
                        onClick={this.savedAddActive.bind(this)}
                      >
                        Cancel
                      </button>
                    </div>
                    <div
                      className={
                        !isMobile()
                          ? "col-md-6"
                          : "col-xs-6 col-md-6 address-submit"
                      }
                    >
                      <button
                        className="btn-blackbg btn-block"
