...
class CartCount extends React.Component {
  ...
  fetchMinicartDetails() {
    apiManager.get(minicartAPI).then(response => {
  ...
  render() {
    ...
    var cartCountItem = null;
    var minicartDropdownItem = null;
    if (CartCount != 0 && CartCount != undefined) {
      cartCountItem = <span className="cartCount">{CartCount}</span>
      minicartDropdownItem = <div className={"dropdown__list " + (this.state.active ? 'dropdown__list--active' : '')}><><div className='mini-cartscroll'>{this.renderOptions()}</div> <button className='checkout-btn'>Checkout</button></></div>
    }

    return (
      <li className="icons mini-cart" onClick={this.handleCartCount}>
        {!isLoading ? (cartCountItem) : (<p className="error">No Cart Item Found</p>)}
