...
import {
  cancelOrderAPI,
  espotReasonOrderItemCancel,
  espotReasonOrderCancel
...
class CancelComponents extends React.Component {
  constructor(props) {
    ...
    this.handleParentStateFromChildState = this.handleParentStateFromChildState.bind(
      this
    );
  }
    ...
    let data = {
      ...
      refundmethod:
        this.state.orderData.paymentMethod === ""
          ...
          : this.state.orderData.paymentMethod,
      cancelreason:
        this.state.value === "Other" ? this.state.text : this.state.value
    };
      ...
      <>
        {this.state.showPopUp === "true" ? (
          ...
          >
            <Modal.Header>
              <h3>
                {this.state.orderItem != undefined ? CANCEL_ITEM : CANCEL_ORDER}
              </h3>
            ...
            <Modal.Body>
              <div className="cancel-order-box">
                <DropDownList
                  ...
                  cancelOrderType={
                    this.state.orderItem != undefined ? "item" : "order"
                  }
                />
                {this.state.error && (
                ...
                <div className="paymet-type">
                  {this.state.orderData.paymentMethod !== "" &&
                    this.state.orderData.paymentMethod !== "COD" && (
                      <p className="msgText">
                        <strong>{MESSAGE_REFUND_METHOD}</strong>
                        {this.state.orderData.paymentMethod}
                      </p>
                    )}
                </div>
