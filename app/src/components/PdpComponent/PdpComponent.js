...
class PdpComponent extends React.Component {
  ...
  getResolveSkuData(resolvedSkuId = null, title = null) {
    ...
    if (this.props.data.type === "product") {
      this.props.data.skuData.map(skuLevelData => {
        if (
          skuId === skuLevelData.partNumber ||
          skuId === skuLevelData.uniqueID
        ) {
          this.getActualResolvedData(
            this.props.data.skuData,
            skuLevelData,
      ...
      resolvedSkuData.defAttributes.map(dataVal => {
        data.map(skuLevelData => {
          skuLevelData.defAttributes.map(attributeValue => {
            if (
              dataVal.values[0].name
                .replace(/[&\/\\#,+()$~%.'":*?<>{}]/g, "")
                .replace(/\s/g, "") ===
              attributeValue.values[0].name
                .replace(/[&\/\\#,+()$~%.'":*?<>{}]/g, "")
                .replace(/\s/g, "")
            ) {
              uniqueId.push(skuLevelData.uniqueID);
              const getValueLength = this.checkArrayFilter(
                uniqueId,
                skuLevelData.uniqueID
              );
      ...
      resolvedSkuData.swatchAttributes.map(dataVal => {
        data.map(skuLevelData => {
          skuLevelData.swatchAttributes.map(attributeValue => {
            if (
              dataVal.values[0].name === attributeValue.values[0].name &&
              uniqueId.indexOf(skuLevelData.uniqueID) === -1
            ) {
              skuDataArr.push(skuLevelData);
              uniqueId.push(skuLevelData.uniqueID);
            }
  ...
  handleSwatches(count) {
    ...
    for (let j = 0; j < selectedSwatches.length; j++) {
      ...
      productSkuData.map(skuLevelData => {
        if (
          this.props.data.type === "kit" ||
          this.props.data.type === "bundle"
        ) {
          skuLevelData.swatchAttributes.map((attributeValue, index) => {
            if (selectedSwatches[j].value === attributeValue.values[0].name) {
              swatches.push(skuLevelData);
            }
          });
        } else if (this.props.data.type === "product") {
          skuLevelData.defAttributes.map((attributeValue, index) => {
            const name = attributeValue.name
              .replace(/\s/g, "")
              .replace(/[&\/\\#,+()$~%.'":*?<>{}]/g, "");
            if (
              selectedSwatches[j].value === attributeValue.values[0].name &&
              name === selectedSwatches[j].name
            ) {
              swatches.push(skuLevelData);
            }
          });
  ...
  handleSelectedSwatches(count) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const getValue = document.querySelector(`input[name = ${name}]:checked`)
        .value;
      selectedSwatches.push({ name: name, value: getValue });
    }
  ...
  callPinCodeAPI(resolvedSkuData, type) {
    ...
    apiManager
      .get(callPincodeApi + pincodeVal, dataParams)
      .then(response => {
        this.setState({
          isLoading: false,
    ...
    if (this.props.matchParams.skuId !== resolvedSkuData.partNumber) {
      ...
      this.props.historyData.replace(
        createSEOPdpURL(
          resolvedSkuData.productName,
          resolvedSkuData.shortDescription,
          resolvedSkuData.partNumber
        )
      );
    }
  }
  ...
  handleScroll() {
    try {
      ...
      var headeroffset = document.getElementById("Pdpstickybar")
        ? document.getElementById("Pdpstickybar").getBoundingClientRect().top
        : null;
      ...
      if (scrollbox2 <= headeroffset) {
        setTimeout(() => {
          if (document.getElementById("topdiv2")) {
            document
              .getElementById("topdiv2")
              .classList.add("slideUpPriceoffer");
            document.getElementById("topdiv2").style.cssText =
      ...
      if (scrollbox2 > headeroffset) {
        document
          .getElementById("topdiv2")
          .classList.remove("slideUpPriceoffer");
      ...
      if (scrollbox3 <= headeroffset) {
        if (document.getElementById("topdiv3")) {
          ...
          document.getElementById("topdiv3").style.cssText =
            "opacity: 1; transition:opacity 1s ease-in-out";
        }
      }
  ...
  render() {
    ...
    if (
      this.state.breadcrumbData !== null &&
      this.props.data !== null &&
      this.props.data !== "" &&
      this.state.breadcrumbData !== undefined &&
      this.state.breadcrumbData.length !== 0
    ) {
      breadcrumbItem = (
        <Breadcrumb
          pdpBreadcrumbPro={this.state.breadcrumbData}
          productNamePro={this.state.skuData.productName}
        />
      );
    }
    ...
    return (
      <>
        {!isLoading && (
          <GSchemas
            schemas={[{ type: "product", product: this.state.skuData }]}
          />
        )}
        {!isLoading && (
          <>
            ...
            {isSticky === true && (
              <div
                className="Pdpstickybar sticky slideup clearfix"
                id="Pdpstickybar"
              >
                <div className="pdpstickyItem clearfix">
                  ...
                  >
                    ...
                    <div className="lockerText">
                      {this.state.skuData.shortDescription}
                    </div>
                  </div>
                    ...
                    <div className="PriceofferCart">
                      <div className="divpriceOffer" id="topdiv2">
                        ...
                        <div className="accessories-offer">
                          <span>
                            {this.state.skuData.discount > 1 && (
                              <>
                                <span className="bold">
                                  {this.state.skuData.discount}% OFF
                                </span>{" "}
                                <br />
                              </>
                            )}
                            {this.state.skuData.promotions.length > 0 &&
                              this.state.skuData.promotions[0].shortDesc && (
                                <>
                                  {parseInt(this.state.skuData.discount) > 1 &&
                                    "& "}
                                  {this.state.skuData.promotions[0].shortDesc}
                                </>
                              )}
          ...
          {!isLoading ? (
            <>
              <Row className="no-margin">
                <Col
                  className="no-paddingLeft img-gallery-box"
                  md={7}
                  sm={12}
                  xs={12}
                >
                  ...
                  <div className="GallerytextBox">
                    <Row>
                      <Col md={12} sm={12} xs={12}>
                        {!isMobile() && (
                          <Col md={7} sm={6} xs={12} className="product">
                        ...
                        {isMobile() && (
                          <Col md={6} sm={12} xs={12} className="product-sku">
                            <span className="text">
                              {this.state.skuData.partNumber}
                            </span>
                          </Col>
                    ...
                    {isMobile() && (
                      <Row className="add-to-cart-floater">
          ...
          <Grid>
            <Row>
              {!isMobile() ? (
                <>
                  {!isLoading && (
                    <ProductFeatures productFeatureData={this.state.skuData} />
                  )}
                </>
              ) : (
                <>
                  {!isLoading && (
                    <MobileProductFeatures
                      productFeatureData={this.state.skuData}
                    />
                  )}
                </>
              )}
            ...
            <Row>
              <Col md={12} sm={12} xs={12} className="purchase-guide-box">
                {!isMobile() ? (
                  <>
                    {this.props.data.type === "product" ? (
                      ...
                      <>
                        {!isLoading &&
                          this.props.data.type !== "product" && (
                            <PurchaseGuide purchaseGuide={this.state.skuData} />
                          )}
                      </>
                    )}
                  </>
                  ...
                  <>
                    {this.props.data.type === "product" ? (
                      <MobilePurchaseGuideGuide
                        purchaseGuide={this.props.data}
                      />
                    ) : (
                      <>
                        {!isLoading &&
                          this.props.data.type !== "product" && (
                            <MobilePurchaseGuideGuide
                              purchaseGuide={this.state.skuData}
                            />
                          )}
                      </>
                    )}
            ...
            <Row>
              {this.props.data.type === "product" ? (
                ...
                <>
                  {!isLoading &&
                    this.props.data.type !== "product" && (
                      <ProductKeywords productKeywords={this.state.skuData} />
                    )}
                </>
              )}
            </Row>
