...
import { homePageLayoutAPI, storeId, accessToken } from '../../../public/constants/constants';
export class HomapegeLayout extends React.Component {
    ...
    getPageLayout() {
        ...
        .then(response => {
            ...
            console.log('HomepageData', response.data.data.GI_Homepage_Layout_Content);
        })
        .catch(error => {
            this.setState({
                error,
    ...
    render() {
        ...
        return !!homePageLayout && (homepageLayout.map((widget, i)=>{
            <WidgetList
                {...widget}
